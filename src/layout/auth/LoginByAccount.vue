...
const handleRememberAccount = () => {
	...
	if (!rememberAccount.value) {
		...
		// 勾选时立即保存当前账号（如果已输入）
		const currentAccount = form.values.account;
		if (currentAccount) {
			localStorage.setItem("rememberedAccount", currentAccount);
		}
	}
};
//提交登录
const onSubmit = form.handleSubmit(async (values) => {
	...
	try {
		...
		if (code == 1) {
			// 记住账号逻辑：如果勾选则保存，否则清除
			if (rememberAccount.value) {
				localStorage.setItem("rememberedAccount", values.account);
			} else {
				localStorage.removeItem("rememberedAccount");
			}
...
onMounted(() => {
	...
	if (savedAccount) {
		form.setValues({ account: savedAccount });
		rememberAccount.value = true; // 同步勾选状态
	}
});
</script>
