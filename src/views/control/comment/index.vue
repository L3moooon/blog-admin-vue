...
import {
  ...
  delComments,
  // changeCommentStatus,
} from "@/api/control/comment/index.ts";
import type { CommentItem, PaginationData } from "@/api/control/comment/type"; // 需根据实际接口定义类型
...
const getList = async () => {
  try {
    ...
    // 调用接口
    const { data, pagination, status } = await getAllComments(params);
    if (status === 1) {
...
const handleDelete = async (id: number) => {
  try {
    ...
    const { status } = await delComments({ id });
